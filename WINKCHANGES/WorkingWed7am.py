...
def index():
    if request.method == "POST":
        ...
        # AUTO-CREATE instructor, slug, and vector store
        slug = _unique_slug(email)

        try:
            vs_id = openai_http.create_vector_store(f"WINK - {email}")
        except Exception as e:
            flash(f"Could not create WINK space. Check OpenAI settings. {e}")
            return render_template_string(TEMPLATE_LOGIN_PAGE)
